...
void TurnLeft(struct DC_motor *mL, struct DC_motor *mR){ 
    ...
    mL->direction = 0; // Set inverse directions for rapid turning
    mR->direction = 1;
    setMotorPWM(mL);
    setMotorPWM(mR);
...
void TurnRight(struct DC_motor *mL, struct DC_motor *mR){
    stop(mL,mR); 
    mL->direction = 1; // Set inverse directions for rapid turning
    mR->direction = 0;
    setMotorPWM(mL);
    setMotorPWM(mR);
...
void PrepareForTurn(struct DC_motor *mL, struct DC_motor *mR){
    move_backward(mL,mR,reverse_time/5); // DECIDE ON FRACTION
    stop(mL,mR);
